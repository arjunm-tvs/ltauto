...
output_folder = script_dir / "trialf_run"
...
for component, metrics in component_data.items():
    ...
    # Default case: all other components
        ...
        with open(filename, mode='w', newline='') as f:
            ...
            for i in range(len(step_conditions)):
                temp, volt = step_conditions[i]
                v_peak  = metrics.get("v_max",   [None]*num_iter)[i]
                nv_peak = metrics.get("nv_max",  [None]*num_iter)[i]
                i_peak  = metrics.get("i_max",   [None]*num_iter)[i]
                ni_peak = metrics.get("ni_max",  [None]*num_iter)[i]
                p_peak  = metrics.get("p_max",   [None]*num_iter)[i]
                np_peak = metrics.get("np_max",  [None]*num_iter)[i]
